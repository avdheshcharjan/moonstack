-- ============================================================================
-- Moonstack Points & Referral System - Complete Database Schema
-- ============================================================================
-- Run this entire script in Supabase SQL Editor
-- This creates all tables, indexes, functions, and policies for the points
-- and referral system including leaderboards and seasons.
-- ============================================================================
...
BEGIN
  IF EXISTS (
    ...
    IF NOT EXISTS (
      ...
      ON user_positions(points_processed) 
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'referrals' AND column_name = 'referrer_wallet'
  ) THEN
    ALTER TABLE referrals ADD COLUMN referrer_wallet TEXT NOT NULL DEFAULT '';
  END IF;
  
  IF NOT EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'referrals' AND column_name = 'referee_wallet'
  ) THEN
    ALTER TABLE referrals ADD COLUMN referee_wallet TEXT NOT NULL DEFAULT '';
  END IF;
  
  IF NOT EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'referrals' AND column_name = 'referee_code_used'
  ) THEN
    ALTER TABLE referrals ADD COLUMN referee_code_used TEXT NOT NULL DEFAULT '';
  END IF;
  
  IF NOT EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'referrals' AND column_name = 'is_active'
  ) THEN
    ALTER TABLE referrals ADD COLUMN is_active BOOLEAN DEFAULT false;
  END IF;
  
  IF NOT EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'referrals' AND column_name = 'first_trade_at'
  ) THEN
    ALTER TABLE referrals ADD COLUMN first_trade_at TIMESTAMPTZ;
  END IF;
  
  IF NOT EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'referrals' AND column_name = 'total_trades_count'
  ) THEN
    ALTER TABLE referrals ADD COLUMN total_trades_count INTEGER DEFAULT 0;
  END IF;
  
  IF NOT EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'referrals' AND column_name = 'total_points_generated'
  ) THEN
    ALTER TABLE referrals ADD COLUMN total_points_generated NUMERIC DEFAULT 0;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    AND constraint_name = 'unique_referee' 
    AND table_name = 'referrals'
  ) THEN
    ALTER TABLE referrals ADD CONSTRAINT unique_referee UNIQUE(referee_wallet);
  END IF;
END $$;

-- Add foreign key constraint separately (after user_points table exists)
-- This ensures the referenced table exists before creating the constraint
DO $$
BEGIN
  ...
  IF EXISTS (
    ...
    -- Check if referrals table has referrer_wallet column
    IF EXISTS (
      SELECT FROM information_schema.columns 
      WHERE table_schema = 'public' 
      AND table_name = 'referrals' 
      ...
      IF EXISTS (
        SELECT FROM information_schema.table_constraints 
        WHERE constraint_schema = 'public'
        AND constraint_name = 'fk_referrer' 
        AND table_name = 'referrals'
      ) THEN
      ...
      BEGIN
        ...
        FOREIGN KEY (referrer_wallet) 
        REFERENCES user_points(wallet_address);
      EXCEPTION WHEN OTHERS THEN
        -- Constraint might already exist or other error - ignore
...
-- Seasons: everyone can read, admins can write (auth handled at API level)
...
-- User points: everyone can read, system can write
...
-- Point transactions: everyone can read, system can write
...
-- Weekly leaderboard: everyone can read, system can write
...
-- Social verifications: everyone can read, system can write
...
-- ============================================================================
DROP MATERIALIZED VIEW IF EXISTS current_season_leaderboard;

-- Create view that handles case where user_positions might not exist
-- This view aggregates points and trade stats for the active season
DO $$
BEGIN
  IF EXISTS (
    SELECT FROM information_schema.tables 
    ...
    -- If user_positions exists, include it in the join
    EXECUTE 'CREATE MATERIALIZED VIEW current_season_leaderboard AS
      SELECT 
        up.wallet_address,
  ...
  ELSE
    -- If user_positions doesn't exist, create view without it
    EXECUTE 'CREATE MATERIALIZED VIEW current_season_leaderboard AS
      SELECT 
        up.wallet_address,
        up.current_season_points,
  ...
  END IF;
EXCEPTION WHEN OTHERS THEN
  -- If materialized view creation fails, create a simple version
  RAISE NOTICE 'Could not create materialized view with user_positions, creating simple version: %', SQLERRM;
  EXECUTE 'CREATE MATERIALIZED VIEW current_season_leaderboard AS
    SELECT 
      up.wallet_address,
      up.current_season_points,
...
-- Add comment for user_positions.points_processed if table exists
DO $$
BEGIN
  IF EXISTS (
    SELECT FROM information_schema.tables 
...
-- ============================================================================
-- Next steps:
-- 1. Verify all tables were created: Check Table Editor in Supabase
-- 2. Create first season: Use POST /api/seasons/create endpoint
-- 3. Set up cron jobs: Configure to call /api/cron/process-points every 15 min
-- 4. Test: Make a trade and verify points are awarded
-- ============================================================================
